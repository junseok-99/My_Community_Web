...
<body>
   ...
   <?php   
        ...
        setcookie("email1",$_POST["email1"],time()+60);
        setcookie("email2",$_POST["email2"],time()+60);
        setcookie("phn1",$_POST["phn1"],time()+60);
        setcookie("phn2",$_POST["phn2"],time()+60);
        setcookie("phn3",$_POST["phn3"],time()+60);
        ...
        elseif($num>0&&$num2>0){
            ...
            if($row["phone_number"]==$row2["phone_number"]&&$row["email"]==$row["email"]){
                ?>
                <div id="mypage_div">
                   <form action="id_view.php" method="post" name="id_view">
                       <ul>
                ...
                echo "<script>
                            alert('입력한 이메일과 핸드폰 번호가 일치하지 않습니다.');
                            history.go(-1);
                    </script>";
            }
